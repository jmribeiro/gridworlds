...
from environments.shared import parse_grid_observation, find_highest_level_apple, goto, compute_vicinity, \
    NOOP, WEST, EAST, SOUTH, NORTH, FORAGE, NUM_ACTIONS, find_closest_level_apple
from environments.utils import sample_discrete_action


class ClosestAppleForager(Agent):
    ...
    def policy(self, obs: Any) -> np.ndarray:
        ...
        apple_id = find_closest_level_apple(location, level, apple_locations, apple_levels, active_apples)
        if apple_id is None:
